...
// social media
if ( ! function_exists( 'hipstyle_social' ) ) {
	function hipstyle_social( $args = array()  ){
		...
		$url = hipstyle_opt('hipstyle_social_url');
...
function hipstyle_contact_form7_shortcode(){

    // contact form list
    $getforms['cs'] = __( 'Custom Shortcode', 'hipstyle' );
    // Instruction
    $Instruction = ''; 
    ...
    if( defined('WPCF7_VERSION') ){
        ...
        if( $loop->have_posts() ){
            ...
            $Instruction = __( 'Contact form not found.', 'hipstyle' );
        }
    }else{
        $url = admin_url( 'plugins.php' );
        
        $Instruction = sprintf( __( 'If you want to use contact form 7, Please install and active contact form 7 plugin. %s Click here to install %s  ' , 'hipstyle' ), '<a target="_blank" href="'.esc_url( $url ).'">', '</a>'  );
    }
...
// Popular post count
function hipstyle_get_post_views($postID){
    $count_key = 'hipstyle_post_views_count';
...
function hipstyle_set_post_views($postID) {
    $count_key = 'hipstyle_post_views_count';
    $count = get_post_meta($postID, $count_key, true);
    if($count==''){
        $count = 0;
...
// blog post categoty 
function hipstyle_get_post_cat(){
    $cats = get_categories();

    $categories = array( 'na' => esc_html__( 'Select post category', 'hipstyle' ) );
