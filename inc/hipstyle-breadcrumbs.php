...
if ( ! function_exists( 'hipstyle_breadcrumbs' ) ) {
    function hipstyle_breadcrumbs( $args = array() ) {
        if ( is_front_page() ) {
            return;
        }
        ...
        $defaults  = array(
            ...
            'home_title'          => esc_html__( 'Home', 'hipstyle' )
        );
        $args      = apply_filters( 'hipstyle_breadcrumbs_args', wp_parse_args( $args, $defaults ) );
        ...
        */
        ...
        $html .= '<li class="breadcrumb-item"><a class="bread-link s-text16 bread-home" href="' . esc_url( get_home_url('/') ) . '" title="' . esc_attr( $args['home_title'] ) . '">'.esc_html__( 'Home', 'hipstyle' ).' </a></li>';
        // Post
        if ( is_singular( 'post' ) ) {
        ...
        $html .= '</ol>';
        $html .= '</nav>';
        $html = apply_filters( 'hipstyle_breadcrumbs_filter', $html );
